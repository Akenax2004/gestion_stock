...
class RolesAndPermissionsSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        // Permissions générales de gestion de stock (exemples)
        ...
        $manageStockIn = Permission::firstOrCreate(['name' => 'manage-stock-in', 'guard_name' => 'web']);
        $manageStockOut = Permission::firstOrCreate(['name' => 'manage-stock-out', 'guard_name' => 'web']);
        ...
        $adminPrincipalRole->givePermissionTo(Permission::all()); // Donne toutes les permissions existantes à l'admin principal

        // Rôle gestion: peut tout gérer sauf les entreprises, licences et gestion des utilisateurs secondaires
        $gestionRole->givePermissionTo([
            $viewProducts, $createProduct, $editProduct, $deleteProduct,
            $manageStockIn, $manageStockOut, $createOrder, $viewReports,
            $manageCustomers, $manageSuppliers, $manageCategories, $manageUnits,
        ...
        $venteRole->givePermissionTo([
            $viewProducts, $manageStockOut, $createOrder, $viewReports, $manageCustomers, $manageQuotations,
        ]);

        // 4. Attribuer le rôle 'admin_principal' au premier utilisateur existant (si applicable)
        // C'est utile si tu as déjà un utilisateur créé et que tu veux qu'il soit l'admin initial.
